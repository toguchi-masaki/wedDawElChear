...
| 🔴 必須 | 4.0 | 入室フロー改善 | ✅ 実装完了 | URL/QR発行→P1待機→P2入室→両者準備完了→ゲーム開始の順序に修正 |
| 🔴 必須 | 4.1 | 降参・切断対応 | 📋 未着手 | 中断時に相手・観戦者へ通知。降参ボタンで強制終了 |
...
- 切断後 30 秒応答がない場合は自動的に降参扱いとする（任意オプション）
...
| 切断検知 | Supabase Presence (`players_${roomId}` チャンネル) でプレイヤーの接続状態を監視 |
| 切断通知 | 切断検知時に相手・観戦者の画面上部に警告バナーを表示（「相手の接続が切断されました」） |
| 自動降参 | 切断後 30 秒以上 Presence が復帰しない場合、相手側クライアントが `SURRENDER` を代理発火（オプション設定） |
| フェーズ追加 | `ABANDONED`: 切断・降参による強制終了時の終了フェーズ（`GAME_OVER` と統合も可） |
