...
export function Scoreboard({ players, setterIdx, chooserIdx, turn, deactivated, history, gameOver = false }: Props) {
  const [tab, setTab] = useState<'detail' | 'grid'>('detail');
  ...
  return (
    <div className="scoreboard-wrap">
      <div className="sb-tab-bar">
        <button className={`sb-tab ${tab === 'detail' ? 'active' : ''}`} onClick={() => setTab('detail')}>詳細</button>
        <button className={`sb-tab ${tab === 'grid' ? 'active' : ''}`} onClick={() => setTab('grid')}>盤面</button>
      </div>
