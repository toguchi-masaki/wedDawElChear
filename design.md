...
| 5 | 上記以外 | Continuation → プレイヤー交代 | 次サイクルへ |

### 5.4 逆転不可の判定（撤廃）

中盤でのスコア差による「逆転不可」早期終了は撤廃した。

**理由:** リード側も今後の選択側ターンでアウトを踏めばスコアが 0 にリセットされうるため、`trailing.score + remainingSum < leading.score` だけでは逆転不可を確定できない。最終的な勝敗は「残り1脚（優先4）」または「8ターン終了（優先3）」時点の確定スコア比較で判定する。これらの時点では最後の脚は選ばれずゲームが終了するため、以降アウトは発生せず確定スコアで正しく比較できる。
