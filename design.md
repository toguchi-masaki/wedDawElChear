...
| 思考時間制限 | オプション機能（ゲーム開始前に設定）。10 / 20 / 30 / 60 秒から選択。攻撃側・守備側ともに適用され、時間切れ時はアクティブなイスからランダム選択 |
...
| `ABORT` | 「中断」ボタン押下。`aborted = true` / `phase = GAME_OVER` に遷移し相手に中断を通知。中断したプレイヤー自身はそのままトップへリダイレクト |

> **廃止:** `START_GAME` / `SHOW_SETTER_SETUP` / `SHOW_CHOOSER_PICK` はマルチデバイス化に伴い削除。ルーム作成・参加はルーティングで処理する。
...
| 🔴 必須 | 4.1 | 降参・切断対応 | ✅ デプロイ完了 | 中断時に相手・観戦者へ通知。降参ボタンで強制終了 |
| 🟡 重要 | 4.2 | 観戦UX改善 | ✅ デプロイ完了 | 途中参加・離脱確認・リアルタイム反映 |
| 🟡 重要 | 4.3 | スコアボード電光掲示板UI | ✅ デプロイ完了 | ターンごとの選択番号＋○×描画。野球電光掲示板風レイアウト |
| 🟡 重要 | 5.1 | スコアボード統合表示 | 📋 未着手 | タブ廃止・詳細と盤面を常時縦並び表示。詳細タブの履歴ログ廃止 |
| 🟡 重要 | 5.2 | 選択中状態のリアルタイム共有 | 📋 未着手 | 確定前の選択番号を Supabase 同期し観戦者・相手に即時反映 |
| 🔴 必須 | 5.3 | スマホUI対応 | 📋 未着手 | モバイルファーストのレスポンシブレイアウト |
...
**要件:**
- 「中断」ボタン押下時に相手へ中断を通知し、相手は中断通知画面からトップへ遷移できる ✅
- Supabase Presence で切断（タブ閉じ・通信断）を検知し、相手に通知バナーを表示 ✅

**実装内容:**

| 項目 | 実装詳細 |
|------|---------|
| 中断アクション | `ABORT` アクション: `aborted = true` / `phase = GAME_OVER` に遷移。Realtime 経由で相手端末に伝播 |
| 中断確認UI | 確認ダイアログ（「ゲームを中断してトップへ戻りますか？」）→ `ABORT` dispatch → 自分はトップへリダイレクト |
| 中断通知 | 相手側は `aborted = true` の `GAME_OVER` フェーズで「相手がゲームを中断しました」を表示し、トップへ戻るボタンを提示 |
| 切断検知 | Supabase Presence (`players_${roomId}_presence` チャンネル) でプレイヤーの接続状態を監視。相手が一度接続後に切断した場合のみ検知（未参加時の誤検知を防止） |
| 切断通知 | 切断検知時に画面上部に警告バナーを表示（「相手の接続が切断されました」） |
| 降参機能 | 撤廃 |

---

### Phase 4.2 — 観戦UX改善　✅ デプロイ完了

**目的:** 観戦体験を向上させる3機能を追加する

#### 4.2.1 観戦離脱機能 ✅

- 観戦画面ヘッダーに「観戦を終了」ボタンを設置
- 押下時に確認ダイアログ（「観戦を終了しますか？」）を表示
- 確認後、トップ画面（`/`）へリダイレクト

#### 4.2.2 途中参加対応 ✅（既存実装で対応済み）

- `useSpectatorRoom` の `init()` でマウント時に Supabase から `game_state` を即時フェッチ済み
- 途中参加した観戦者も最新状態から観戦を開始できる

#### 4.2.3 リアルタイム反映 ✅（既存実装で対応済み）

- `useSpectatorRoom` が `postgres_changes` イベントを購読し、プレイヤーの操作をリアルタイムで反映済み

---

### Phase 4.3 — スコアボード電光掲示板UI　✅ デプロイ完了

**目的:** ターンごとの選択結果を野球の電光掲示板風に一覧表示し、ゲームの流れを視覚的に把握しやすくする

**要件:**
- 縦軸: ターン（T1〜T8）、横軸: プレイヤー2人 ✅
- 各セルに選択した番号を表示し、○（セーフ）または ×（アウト）を描画 ✅
...
| ○×描画 | セルに番号テキスト＋○（緑）/ ×（赤）を並列表示。セーフは緑・アウトは赤の背景色でも識別可能 |
| アウト累計 | 各プレイヤー列のフッターにアウト数（●●● 形式）を表示 |
| スコア合計 | 各プレイヤー列のフッターに現在のスコアを表示 |
| 現スコアボード | タブ切替で併存（「詳細」タブ: 従来の Scoreboard / 「盤面」タブ: ScoreboardGrid） |

---

### Phase 5.1 — スコアボード統合表示　📋 未着手

**目的:** タブ切替の操作コストをなくし、詳細情報と盤面を常に同時に把握できるようにする

**要件:**
- `Scoreboard`（詳細）と `ScoreboardGrid`（盤面）のタブ切替を廃止し、縦に並べて常時表示
- 詳細タブ内の選択履歴ログ（TurnRecord 時系列リスト）を廃止
  - 選択履歴の情報は `ScoreboardGrid` の盤面セルで代替されるため冗長
- スコア・アウトカウント・残りアクティブイス合計値などの集計情報は `ScoreboardGrid` のヘッダーまたはフッターに統合

**実装内容:**

| 項目 | 実装詳細 |
|------|---------|
| タブ廃止 | `Scoreboard` と `ScoreboardGrid` を包むタブ UI を削除。両コンポーネントを縦方向に並べてレンダリング |
| 履歴ログ廃止 | `Scoreboard` コンポーネントから TurnRecord リストの描画を削除 |
| 情報統合 | スコア・アウトカウントを `ScoreboardGrid` 内に集約し、`Scoreboard` はプレイヤー名・ターン数・残りイス合計値のみ表示（または `ScoreboardGrid` に完全統合） |

> **注意:** `GameState.history`（`TurnRecord[]`）のデータ自体は削除しない。`ScoreboardGrid` が参照するため引き続き必要。

---

### Phase 5.2 — 選択中状態のリアルタイム共有　📋 未着手

**目的:** 守備側が確定ボタンを押す前の「悩んでいる状態」を観戦者・攻撃側にも見せ、緊張感とライブ感を高める

**要件:**
- 守備側がイスをタップした時点（確定前）で選択中番号を Supabase に即時同期
- 観戦者画面・攻撃側の待機画面でも選択中のイスをハイライト表示
- 確定ボタン押下で `CHOOSE` アクションに進む現行フローは変更しない
- 守備側がタップを変更した場合は上書き同期する

**GameState 追加フィールド:**

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `pendingPick` | number \| null | 守備側が確定前に選択中のイス番号。`CHOOSE` 確定時または `CONTINUE` 時に null リセット |

**実装内容:**

| 項目 | 実装詳細 |
|------|---------|
| フィールド追加 | `GameState` に `pendingPick: number \| null` を追加。初期値 `null` |
| 同期タイミング | `ChooserPickScreen` でイスをタップするたびに `pendingPick` を更新し Supabase へ書き込み（`SETTER_DONE` 相当の即時 upsert） |
| リセット | `CHOOSE` アクション確定時・`CONTINUE` アクション時に `pendingPick = null` |
| 表示 | `LengeGrid` で `pendingPick` に該当するイスをパルスアニメーション付きでハイライト。観戦者・攻撃側待機画面にも反映 |
| 帯域対策 | タップのたびに書き込むため、500ms のデバウンスを設けて Supabase への書き込み回数を抑制 |

---

### Phase 5.3 — スマホUI対応　📋 未着手

**目的:** スマートフォンの画面幅（360px〜430px 相当）でも操作・閲覧が快適にできるようにする

**要件:**
- 全画面でモバイルファーストのレスポンシブレイアウトを適用
- `LengeGrid`（イスボタングリッド）のボタンがタップしやすいサイズを維持
- `ScoreboardGrid` が狭い画面でも見切れずに表示される
- テキスト・ボタンが極端に小さくならない
...
| 項目 | 実装詳細 |
|------|---------|
| ブレークポイント | Tailwind の `sm`（640px）を境界とし、モバイルはシングルカラム、PC はサイドバイサイド配置 |
| `LengeGrid` | ボタンサイズを `min-w-[44px] min-h-[44px]`（iOS HIG 推奨タップ領域）以上に設定。グリッド列数はモバイルで4列・PCで6列 |
| `ScoreboardGrid` | モバイルで横スクロール可能なコンテナ（`overflow-x-auto`）に包む。または列幅を圧縮してスタック表示 |
| スコアボード統合表示 | Phase 5.1 の縦並びレイアウトをモバイルでも維持。画面高さが不足する場合はスクロール可能にする |
| フォント・余白 | モバイルでは `text-sm` / `p-2` 等を基準に余白を縮小。PC では現行レイアウトを維持 |
| QRコード画面 | `LobbyScreen` のQR表示サイズをモバイルで調整（180px → 150px 等）し、テキストが折り返さないようにする |
